...
4. **Mid-session refresh (Charles, Promax Critic):** writing `config.yaml` alone does not update Desktop `currentPersonality` (sticky `prev || personality` while the session is active). After companion apply, refresh the **focused** live session — not the active gateway socket. Desktop tab/tile focus can move without swapping that socket, so `host.request('config.set')` often hit **default** while Critic was focused (YAML looked correct; ephemeral prompt never changed). A later hotfix called `host.requestProfile("mechanic")` with a **bare profile string**; Charles’s Desktop has multiple registry sources, so the SDK rejects profile-only routing and requires a full `PluginProfileRoute` from `host.profileRoutes()`: `{ connectionId, mode, profile, targetProfile }`. Resolve session via `focusedSessionId` **or** `activeSessionId`; resolve the route via `focusedSessionOwner` + `profileRoutes()` (connectionId+profile, else unique name, else synthesize from owner). **Never** pass a bare profile string to `requestProfile`. Fall back to `host.request` only when the focused profile matches the active gateway profile (or is empty). Live-applied **only** if `result.info != null`. Still **no** `host.newChat`.
5. Next **user** New Chat restores stash (personality + user `agent.system_prompt` + TTS). Catalog dicts may remain; they do nothing unless selected.
...
| Path | What changed |
|---|---|
| `desktop/plugin.js` | Unified apply through **session overlay**. Apply / Standard Hermes / startup **never** call `host.newChat`. After apply/reset, mid-session refresh uses `host.profileRoutes()` + `host.requestProfile(fullRoute, 'config.set', personality)` (never a bare profile string). Watcher resets **only** on stored-id New Chat. **Per-profile titlebar.** Titlebar lists **complete persona packs only**. Studio is a top-to-bottom workflow; selecting a pack hydrates the form; Save **PUT**s existing packs (strength persists) and stays open. |
...
|---|---|---|
| Titlebar pick complete persona pack | Stash `display.personality` + user `agent.system_prompt` + TTS; apply pack **character strength 0–100** (0 = no overlay; 1–99 style blend; 100 = eclipse identity). Fish-prefer TTS. | **Stay in this chat.** Then `requestProfile(full PluginProfileRoute, config.set personality)` from `host.profileRoutes()`. Live-applied only if `result.info != null`. **No** `host.newChat` |
| Titlebar voice-only clone | **Not listed.** Orphans stay in Studio for clone/edit. No apply without a pack. | — |
| Titlebar Standard Hermes | Restore stash, or Edge stock if no usable stash. Never `voicebox.voice: default` | `config.set` personality = `none` on focused session. **No** `host.newChat` |
...
- [ ] Apply and Standard Hermes clear do **not** call `host.newChat`; mid-session refresh uses `host.profileRoutes()` + `host.requestProfile(fullRoute, 'config.set', …)` (never a bare profile string); live-applied only if `result.info != null`
