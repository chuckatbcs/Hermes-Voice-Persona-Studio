...
4. **Mid-session refresh (Charles, Promax Critic):** writing `config.yaml` alone does not update Desktop `currentPersonality` (sticky `prev || personality` while the session is active). After companion apply, refresh the **focused** live session — not the active gateway socket. Desktop tab/tile focus can move without swapping that socket, so `host.request('config.set')` often hit **default** while Critic was focused (YAML looked correct; ephemeral prompt never changed). Resolve session via `focusedSessionId` **or** `activeSessionId`; resolve profile via `focusedSessionOwner.profile` → `focusedSessionProfile` → `focusedProfile()`. Prefer `host.requestProfile(targetProfile, 'config.set', { key:'personality', value: <catalog key e.g. eric_cartman>, session_id })`; fall back to `host.request` only if `requestProfile` is missing. Gateway `_set_personality` can return success without applying — treat live-applied **only** if `result.info != null` (`history_reset` without `info` is a failure). Warn on skipped no-session / no-request too. Still **no** `host.newChat`.
...
| Path | What changed |
|---|---|
| `desktop/plugin.js` | Unified apply through **session overlay**. Apply / Standard Hermes / startup **never** call `host.newChat`. After apply/reset, **mid-session `host.requestProfile(profile, 'config.set', personality)`** refreshes the focused live session (not the active gateway socket). Watcher resets **only** on stored-id New Chat. **Per-profile titlebar.** Titlebar lists **complete persona packs only**. Studio is a top-to-bottom workflow; selecting a pack hydrates the form; Save **PUT**s existing packs (strength persists) and stays open. |
...
|---|---|---|
| Titlebar pick complete persona pack | Stash `display.personality` + user `agent.system_prompt` + TTS; apply pack **character strength 0–100** (0 = no overlay; 1–99 style blend; 100 = eclipse identity). Fish-prefer TTS. | **Stay in this chat.** Then `requestProfile(focused profile, config.set personality)` with runtime session id (`focusedSessionId` or `activeSessionId`). Live-applied only if `result.info != null`. **No** `host.newChat` |
...
- [ ] Apply and Standard Hermes clear do **not** call `host.newChat`; mid-session refresh uses `host.requestProfile(focused profile, 'config.set', { key:'personality', value: catalog key or none, session_id })` (not the active gateway socket); live-applied only if `result.info != null`
