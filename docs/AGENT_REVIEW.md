...
4. **Mid-session refresh (Charles, Promax Critic):** writing `config.yaml` alone does not update Desktop `currentPersonality` (sticky `prev || personality` while the session is active). After companion apply, the plugin calls `host.request('config.set', { key: 'personality', value: <catalog key e.g. eric_cartman>, session_id: focusedSessionId() })` so the gateway `_apply_personality_to_session` refreshes the live ephemeral prompt without history reset. After Standard Hermes / `session/reset`, the same RPC uses `value: 'none'`. Skip when there is no runtime session id (draft). Still **no** `host.newChat`.
...
| Path | What changed |
|---|---|
| `desktop/plugin.js` | Unified apply through **session overlay**. Apply / Standard Hermes / startup **never** call `host.newChat`. After apply/reset, **mid-session `host.request('config.set', personality)`** refreshes the focused live session. Watcher resets **only** on stored-id New Chat. **Per-profile titlebar.** Titlebar lists **complete persona packs only**. Studio is a top-to-bottom workflow; selecting a pack hydrates the form; Save **PUT**s existing packs (strength persists) and stays open. |
...
- [ ] Apply and Standard Hermes clear do **not** call `host.newChat`; mid-session refresh uses `host.request('config.set', { key:'personality', value: catalog key or none, session_id })` when a runtime session id exists
