...
4. **Mid-session refresh (Charles, Promax Critic):** writing `config.yaml` alone does not update Desktop `currentPersonality` (sticky `prev || personality` while the session is active). After companion apply, refresh the **focused** live session. Read the active gateway from `host.state.profile` (SDK `$activeGatewayProfile`), then legacy names. **Prefer ambient `host.request`** when the focused profile matches that gateway (case-insensitive) — secondary `requestProfile` dials can miss in-memory sessions. Use `requestProfile(fullRoute)` only when ambient request is not gateway-safe. Full `PluginProfileRoute` from `host.profileRoutes()` (`{ connectionId, mode, profile, targetProfile }`); **never** a bare profile string. Live-applied **only** if `result.info != null`. Success toast distinguishes live refresh vs config-only. Still **no** `host.newChat`.
...
| `backend/bot_profiles.py` | Surgical writes; refuse Voicebox id `default`; catalog `system_prompt` is the style overlay (`source: hermes-personastudio`). Fish bind always pins `--fish-voice <id>` (plus `--fish-label` / `clones.<label>`) and syncs `~/.hermes/fish_voices.json` so a stale label cache cannot 400 a deleted Fish model. |
